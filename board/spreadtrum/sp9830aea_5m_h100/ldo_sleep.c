...
static int self_adaption_dcdcarm_via_efuse(void)
{
	int Dhryst_data =0,v_arm =0;

	sci_efuse_Dhryst_binning_get(&Dhryst_data);
	v_arm = regulator_get_voltage("vddarm");

	if( Dhryst_data <= 28 && Dhryst_data >= 22)
		regulator_set_voltage("vddarm",v_arm + 50);

	return 0;
}
static int init_ldo_voltage(void)
{
    self_adaption_dcdcarm_via_efuse();
