...
#define PROP_NAME_KEY	"sprd,name"
#define CPROC_PROP_VAL_KEY	"cptl"
#define SIPC_PROP_VAL_KEY	"sipc-lte"

#define PUBCP_IRAM_ADDRESS 0x1a000
...
extern int fdt_get_cp_loader(void *fdt, char *pname, char *pval, u32 *count, u32 *loader);
static inline void pubcp_boot(void)
{
	...
	if(fdt_get_cp_loader((u8 *)DT_ADR, PROP_NAME_KEY, CPROC_PROP_VAL_KEY, &count, loader) == 0) {
		printf("%s: cp boot code from dtb\n", __func__);
...
{
	*((volatile u32*)REG_PMU_APB_SYS_SOFT_RST) |= BIT_1;   /* reset M3 */
}
...
//  Note:
/*****************************************************************************/
static inline void pmic_arm_m3_boot(void)
{
    u32 state;
	*((volatile u32*)REG_PMU_APB_SLEEP_CTRL) &= ~BIT_23;   /*clear M3 force sleep */
	*((volatile u32*)REG_PMU_APB_SYS_SOFT_RST) &= ~(BIT_1 | BIT_21);   /* reset M3 */
    msleep(50);
}
