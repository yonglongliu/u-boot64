...
void board_keypad_init(void)
{
	...
	if(0 == get_hw_version())
	{
		sprd_gpio_request(NULL, VOLUMUP_KEY_GPIO);
		sprd_gpio_direction_input(NULL, VOLUMUP_KEY_GPIO);
	}
...
unsigned char board_key_scan(void)
{
    ...
    /*Adie EIC10 as Key_Volumedown*/
	int volumedown = -1;
	sprd_eic_request(EIC_KEY2_7S_RST_EXT_RSTN_ACTIVE);
	udelay(3000);
	volumedown = sprd_eic_get(EIC_KEY2_7S_RST_EXT_RSTN_ACTIVE);
	if(volumedown < 0)
	...
	if (0 == get_hw_version())
	{
	    uint32_t gpio_volumeup = -1;
		gpio_volumeup = sprd_gpio_get(NULL, VOLUMUP_KEY_GPIO);
		if(0 == gpio_volumeup){
			key_code = KEY_VOLUMEUP;
			printf("[gpio keys] volumeup is pressed\n");
		}
	}
    return key_code;
}
