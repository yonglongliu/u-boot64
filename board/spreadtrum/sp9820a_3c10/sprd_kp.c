...
//      gpio_tmp = gpio_scan();
	...
	//try to get low level
	for (i = 0; i < 6; i++) {
		if (gpio_tmp = gpio_scan())
			break;
		udelay(250);
	}
...
//		printf("[eic keys] volumeup : sprd_eic_get return ERROR!\n");
	//DAP_UP -->volumeup[2,2]+GPIO9
	if(0x2 == sprd_key_map->row && 0x02 == sprd_key_map->col
	   && 3 == gpio_tmp) {
		key_code = KEY_VOLUMEUP;
		printf("key volumeup pressed!\n");
	} else if (0x0 == sprd_key_map->row && 0x02 == sprd_key_map->col
		   && 2 == gpio_tmp) {//DAP_DOWN-->volumedown, [0,2],GPIO8
		key_code = KEY_VOLUMEDOWN;
		printf("key volumedown pressed!\n");
	}
