...
void init_ldo_sleep_gr(void)
{
	...
	switch (hw_version) {
		...
		case 1:
			// enable ldocamd to prevent leakage, because gps module is changed
			enable_ldo_camd = 1;
			break;
		...
		default:
			// enable ldocamd to prevent leakage, because gps module is changed
			enable_ldo_camd = 0;
			// use TCXO intead of VC-TCXO, so cp PLL should be changed to sine1
			cp_on_sine1 = 0;
			break;
	}
	...
	ANA_REG_SET(ANA_REG_GLB_LDO_PD_CTRL,
		...
		BIT_LDO_CAMMOT_PD |
		BIT_LDO_CAMIO_PD |
		(enable_ldo_camd ? 0 : BIT_LDO_CAMD_PD) |
		BIT_LDO_CAMA_PD |
		BIT_LDO_SIM2_PD |
		BIT_LDO_SIM1_PD |
		BIT_LDO_SIM0_PD |
	...
	reg_val = ANA_REG_GET(ANA_REG_GLB_LDO_SHPT_PD2);
	reg_val|= (1 << 0);
	ANA_REG_SET(ANA_REG_GLB_LDO_SHPT_PD2,reg_val);
	...
	ANA_REG_SET(ANA_REG_GLB_PWR_SLP_CTRL1,
		...
		(enable_ldo_camd ? 0 : BIT_SLP_LDOCAMD_PD_EN) |
	...
	*************************************************/
	...
	CHIP_REG_SET(REG_PMU_APB_LTEPLL_REL_CFG,
		(cp_on_sine1 ? BIT_LTEPLL_REF_SEL : 0) |
		//BIT_LTEPLL_ARM7_SEL |
		BIT_LTEPLL_VCP1_SEL |
		//BIT_LTEPLL_VCP0_SEL |
		//BIT_LTEPLL_CP1_SEL |
	...
	CHIP_REG_SET(REG_PMU_APB_TWPLL_REL_CFG,
		(cp_on_sine1 ? BIT_TWPLL_REF_SEL : 0) |
		BIT_TWPLL_ARM7_SEL |
		BIT_TWPLL_VCP1_SEL |
		//BIT_TWPLL_VCP0_SEL |
		//BIT_TWPLL_CP1_SEL |
