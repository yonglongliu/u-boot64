...
void init_ldo_sleep_gr(void)
{
	...
	reg_val = ANA_REG_GET(ANA_REG_GLB_LDO_SHPT_PD1);
	reg_val|= (1 << 11);
	ANA_REG_SET(ANA_REG_GLB_LDO_SHPT_PD1,reg_val);

	reg_val = ANA_REG_GET(ANA_REG_GLB_LDO_SHPT_PD2);
	reg_val|= (1 << 7);
	ANA_REG_SET(ANA_REG_GLB_LDO_SHPT_PD2,reg_val);

      //vddrf 1.5 -> 1.8v for 3532 bug fix
	ANA_REG_SET(ANA_REG_GLB_DCDC_RF_ADI, 0x180);

	/* 850/900 swtich module, need open kpled. */
	ANA_REG_SET(ANA_REG_GLB_KPLED_CTRL, 0xca0);
	...
	****************************************/
	...
	ANA_REG_SET(ANA_REG_GLB_PWR_XTL_EN3,
        ...
        //bit4-5: 0x10. quick dischrg
        reg_val = ANA_REG_GET(ANA_REG_GLB_DCDC_DISCHRG);
        reg_val |= (0x1 << 5);
        reg_val &= 0xFFFFFFEF;
        ANA_REG_SET(ANA_REG_GLB_DCDC_DISCHRG, reg_val);
	...
	*************************************************/
	...
	CHIP_REG_SET(REG_PMU_APB_WIFIPLL2_REL_CFG,
		//BIT_WIFIPLL2_REF_SEL |     //GPU select sin0 source
...
#if 0  //it has been configured in chipram
	...
	REG32(REG_PMU_APB_CGM_GPU_MM_AUTO_GATE_EN) &= ~(
		BIT_CGM_WIFIPLL2_GPU_AUTO_GATE_EN |
		BIT_CGM_153_6M_GPU_AUTO_GATE_EN |
		BIT_CGM_384M_GPU_AUTO_GATE_EN |
		BIT_CGM_460_8M_GPU_AUTO_GATE_EN |
		BIT_CGM_256M_GPU_AUTO_GATE_EN |
		BIT_CGM_208M_GPU_AUTO_GATE_EN |
		BIT_CGM_312M_GPU_AUTO_GATE_EN |
		BIT_CGM_300M_GPU_AUTO_GATE_EN);

	REG32(REG_PMU_APB_CGM_GPU_MM_EN) &= ~(
		BIT_CGM_WIFIPLL2_GPU_EN |
		BIT_CGM_153_6M_GPU_EN |
		BIT_CGM_384M_GPU_EN |
		BIT_CGM_460_8M_GPU_EN |
		BIT_CGM_208M_GPU_EN |
		BIT_CGM_312M_GPU_EN |
		BIT_CGM_300M_GPU_EN);
	...
	CHIP_REG_SET(REG_PMU_APB_XTL_WAIT_CNT,
		BITS_XTL1_WAIT_CNT(0x73)		|
		BITS_XTL0_WAIT_CNT(0x73)		|
		0
	);
