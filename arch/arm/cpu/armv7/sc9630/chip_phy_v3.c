...
#define SP9832A_CHIP_ID          (0x96301000)
#define SP9832A_CHIP_ID_MASK     (0xFFF)
#define SP9820A_CHIP_ID          (0x96320001)
#define DO_SRAM_REPAIR  0x20000
#define NOT_SRAM_REPAIR 0x0
#define SRAM_REPAIR_MASK 0x20000
#define NEED_SRAM_REPAIR 0x01
...
static void sram_repair(unsigned int para)
{
	...
	uint32 sram_flag;
	chip_id = __raw_readl(REG_AON_APB_AON_CHIP_ID);
	if(SP9832A_CHIP_ID == (chip_id & ~SP9832A_CHIP_ID_MASK))  //sp9832a momory repair
	{
		sram_flag = (__ddie_efuse_read(3) & SRAM_REPAIR_MASK);
		/* if the value of Block3 Bit17 is 1, it needs memory repair */
		if(sram_flag == DO_SRAM_REPAIR)
		{
			CHIP_REG_AND(REG_PMU_APB_BISR_BYP_CFG, ~(BIT_PD_CP1_LTE_P2_BISR_FORCE_BYP |\
						BIT_PD_CP1_LTE_P1_BISR_FORCE_BYP |\
						BIT_PD_CP1_CEVA_BISR_FORCE_BYP |\
						BIT_PD_CP0_HU3GE_BISR_FORCE_BYP |\
						BIT_PD_CP0_TD_BISR_FORCE_BYP |\
						BIT_PD_CP0_CEVA_0_BISR_FORCE_BYP |\
						BIT_PD_MM_TOP_BISR_FORCE_BYP));
		}
	}
	if(SP9820A_CHIP_ID == chip_id)  //pikel memory repair
	{
		sram_flag = (__ddie_efuse_read(3) >> 17);
		sram_flag = (sram_flag & NEED_SRAM_REPAIR);
		/* if the value of Block3 Bit17 is 1, it needs memory repair */
		if(sram_flag)
		{
			CHIP_REG_AND(REG_PMU_APB_BISR_BYP_CFG, ~(BIT_PD_CP1_LTE_P1_BISR_FORCE_BYP |\
						BIT_PD_CP1_CEVA_BISR_FORCE_BYP |\
						BIT_PD_CP0_TD_BISR_FORCE_BYP |\
						BIT_PD_CP0_CEVA_0_BISR_FORCE_BYP |\
						BIT_PD_MM_TOP_BISR_FORCE_BYP));
