...
static dir_entry *find_directory_entry(fsdata *mydata, int startsect,
	const char *filename, dir_entry *retdent, __u32 start)
{
	__u32 curclust = (startsect - mydata->data_begin) / mydata->clust_size;

	debugf("get_dentfromdir: %s\n", filename);
	...
	while (1) {
		...
		for (i = 0; i < DIRENTSPERCLUST; i++) {
			...
			debugf("DentName: %s\n", s_name);
			debugf(", start: 0x%x", START(dentptr));
			debugf(", size:  0x%x %s\n",
		...
		if (CHECK_CLUST(curclust, mydata->fatsize)) {
			debugf("curclust: 0x%x\n", curclust);
			debugf("Invalid FAT entry\n");
			return NULL;
		}
	}
 ...
 *
 */
static int do_fat_write(const char *path_name, void *buffer, unsigned long size)
{
	...
	while(*cur != '\0') {
		...
		if (retdent) {
			...
			ret = set_cluster(mydata, dir_curclust,
				...
				mydata->clust_size * mydata->sect_size);
			if (ret) {
				debugf("Error: writing directory entry\n");
				goto exit;
			}

			if (get_cluster(mydata, start_cluster, get_dentfromdir_block,
				mydata->clust_size * mydata->sect_size) != 0) {
				debugf("Error: reading directory block\n");
				goto exit;
			}
...
int check_stat(const char *dirname)
{
	dir_entry *dentptr = NULL;
	dentptr = check_folder_flag(dirname);
	if (dentptr == NULL)
		return 1;
	return 0;
}

int fat_write_file(const char *filename, void *buf, int offset, unsigned long len)
{
	file_fat_write(filename, buf, len);
}