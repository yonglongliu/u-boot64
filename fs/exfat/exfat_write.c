	...
	if (needed_clust) {
		...
		if (prim_entry==NULL) {
			...
			if (p_ent->bf1.pbuf==NULL) {
				p_ent->bf1.pbuf = memalign(ARCH_DMA_MINALIGN, mydata->sect_size);
				if (p_ent->bf1.pbuf == NULL) {
					debugf("Error: allocate memory failed.\n");
					return NULL;
				}
					...
					char *filename, int is_dir)
{
	...
	if (p_parent->strext == NULL) {// root dir
		sec_all = (__u32)(EXFAT2CPU64(p_parent->strext->data_len)/mydata->sect_size);
...
exfat_path_walk(exfsdata *mydata, const char *complete_filename, exfat_whole_dentry *file_ent)
{
	...
	while (*cur != 0) {
		...
		if (is_dir) {
			...
			if(parent.bf0.pbuf)
				free(parent.bf0.pbuf);
			if(parent.bf1.pbuf)
				free(parent.bf1.pbuf);
...
exit:
	...
	if(parent.bf1.pbuf)
		free(parent.bf1.pbuf);
	if(current.bf0.pbuf)
		free(current.bf0.pbuf);
	if(current.bf1.pbuf)
		free(current.bf1.pbuf);
...
exit:
	...
	if(file_ent.bf0.pbuf)
		free(file_ent.bf0.pbuf);
	if(file_ent.bf1.pbuf)
		free(file_ent.bf1.pbuf);
