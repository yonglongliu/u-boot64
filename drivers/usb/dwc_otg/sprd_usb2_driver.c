...
LOCAL __align(64) uint32_t    s_setup_packet[8] = {0};
...
/*--------------------------- Local Data ------------------------------------*/
static __align (4) const uint8_t DeviceDescrSerial[] =
{
...
static __align (4) const uint8_t ConfigDescrSerial_64[]  =
{
...
{
	unsigned int ret, phy_tune1, tfhres;
	extern int sci_efuse_usb_phy_tune_get(unsigned int *);

	ret = sci_efuse_usb_phy_tune_get(&tfhres);
	if (ret)
		tfhres = tfhres > 6 ? tfhres - 6 : 0;
	phy_tune1 = * (volatile uint32_t *)(REG_AP_APB_USB_CTRL1);
	phy_tune1 &= ~0xff;
	phy_tune1 |= (tfhres << 3) | 0x2;
	printf("%s, tfhres: 0x%x, phy_tune1: 0x%x\n",
		__func__, tfhres, phy_tune1);
	* (volatile uint32_t *)REG_AP_APB_USB_CTRL1 = phy_tune1;
	* (volatile uint32_t *)REG_AP_APB_USB_CTRL0 = USB_PHY_TUNE_VALUE;
}
#endif
	if (autodl_mode) {
