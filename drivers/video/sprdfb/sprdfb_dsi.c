...
int32_t sprdfb_dsi_init(struct sprdfb_device *dev)
{
	...
	unsigned int dsi_cycle_period_n = 0;
	struct info_mipi * mipi = dev->panel->info.mipi;
	int i = 0;

#ifdef CONFIG_FPGA   //not use lcd
	...
	dsi_early_int();
	dsi_cycle_period_n = 8000 / (mipi->phy_feq / 1000);
	...
	dsi_instance->max_bta_cycles = (2000 + 6600) / dsi_cycle_period_n ;//10M 2000ns BTA 6600 panel read back time.
