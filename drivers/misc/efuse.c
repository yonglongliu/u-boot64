...
int soc_is_sharkls(void)
{
	 if((Get_chip_id() & SCX9830I_ALPHA_TAPOUT_MASK) ==
	 (SCX9830I_ALPHA_TAPOUT & SCX9830I_ALPHA_TAPOUT_MASK)){
		return 1;
	 }else{
		return 0;
	 }
...
void efuse_init(void)
{
	...
	efuse_reset();
	cfg0 = __raw_readl((void *)REG_EFUSE_CFG0);
#if defined(CONFIG_SP7731C) || defined(CONFIG_ARCH_SCX20L)
	cfg0 |= BITS_EFS_TYPE(0x2);
#endif
    if(soc_is_sharklc()){
		cfg0 |= BITS_EFS_TYPE(0x2);
