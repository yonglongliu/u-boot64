...
static int max_discard_sector=1024;
...
struct mmc *find_mmc_device(int dev_num)
{
 ...
 static unsigned int mmc_mmc_erase_timeout(struct mmc *card,
						   unsigned int arg, unsigned int qty)
 {
	 ...
	 if ((arg == DISCARD_ARG )||
		 (arg == TRIM_ARG)) {
		 erase_timeout = card->discard_timeout;
	 }  else
		erase_timeout = card->erase_grp_timeout;
 ...
 static unsigned int mmc_do_calc_max_discard(struct mmc *mmc,
						 unsigned int arg)
 {
	 ...
	 max_qty = UINT_MAX / mmc->erase_grp_size;
	 max_discard_to = (1 << 30) / (mmc->clock / 1000);
	 /* Find the largest qty with an OK timeout */
	 do {
		 y = 0;
		 for (x = 1; x && x <= max_qty && max_qty - x >= qty; x <<= 1) {
			 timeout = mmc_mmc_erase_timeout(mmc, arg, qty + x);
			 if (timeout > max_discard_to)
				 break;
	 ...
	 max_discard = --qty * mmc->erase_grp_size;
 ...
 unsigned long mmc_berase(int dev_num, lbaint_t start, lbaint_t blkcnt)
 {
	 ...
	 while (blk < blkcnt) {
		 blk_r = ((blkcnt - blk) > max_discard_sector) ?
			max_discard_sector : (blkcnt - blk);
...
static int mmc_startup(struct mmc *mmc)
{
		...
		if ((ext_csd[EXT_CSD_PARTITIONING_SUPPORT] & PART_SUPPORT) &&
			...
			mmc->erase_grp_size = (erase_gsz + 1)
				* (erase_gmul + 1);
                  ...
                  mmc->discard_timeout = 300 * ext_csd[EXT_CSD_TRIM_MULT];
		mmc->erase_grp_timeout = 300 *  ext_csd[EXT_CSD_ERASE_TIMEOUT_MULT];
	...
	mmc->erase_cmd = ERASE_ARG;
	if (!IS_SD(mmc)) {
		max_discard_sector = mmc_do_calc_max_discard(mmc,ERASE_ARG);
		printf("for group erase max_discard_sector is:%d\n",max_discard_sector);
		if (ext_csd[EXT_CSD_SEC_FEATURE_SUPPORT] & EXT_CSD_SEC_GB_CL_EN) {
			max_discard_sector = mmc_do_calc_max_discard(mmc,TRIM_ARG);
			mmc->erase_cmd = TRIM_ARG;
		}
	}
	printf("last  max_discard_sector is:%d mmc->erase_cmd:0x%x\n",max_discard_sector,mmc->erase_cmd);
