...
uint32_t get_code_size(uint8_t * header_addr)
{
	int i = 0;
	header_info_t *header_p = (header_info_t *) header_addr;
	...
	for (i = 0; i < header_p->tags_number; i++) {
		if (header_p->tag[i].tag_name == CODE_NAME) {
			return header_p->tag[i].tag_size;
		}
	}
	return 0;
}
...
void *get_user_puk_addr(uchar * name, uint8_t * header_addr)
{
	int i = 0;
	void *addr = NULL;
	header_info_t *header_p = (header_info_t *) header_addr;
...
int secure_verify(uchar * name, uint8_t * header, uint8_t * code)
{
	...
	header_info_t * secboot_signed_hdr = header;
	boot_img_hdr * boot_img_hdr = header;
	unsigned int boot_image_flag = FALSE;
	uint32_t chk_ret = 0;
	int sec_time_count = 60;

	debugf("%s, name: %s enter\r\n", __FUNCTION__, name);
	...
	debugf("header =%x, \n puk_addr=%x, \n vlr_addr=%x, \n code_addr=%x, \n", header, puk_addr, vlr_addr, code_addr);

	extern int loader_binding_data_set(unsigned char * UBootHeadr, unsigned char * KeyData, unsigned int KeyLen);
	loader_binding_data_set(header, puk_addr, BINDING_KEY_MAX_LEN);

	/*update boot_img_type*/
	/*image header check */
	extern uint32_t boot_img_type;
	if (!memcmp(boot_img_hdr->magic, BOOT_MAGIC, BOOT_MAGIC_SIZE)) {
		boot_img_type = BOOTIMAGE_UNSIGNED;
		boot_image_flag = TRUE;
	}
	else if (secure_header_parser( (u8 *) secboot_signed_hdr)) {
		boot_img_hdr = code_addr;
		if(!memcmp(boot_img_hdr->magic, BOOT_MAGIC, BOOT_MAGIC_SIZE)) {
			boot_image_flag = TRUE;
			if (secboot_signed_hdr->tags_number == 3) {
				boot_img_type = BOOTIMAGE_SIGNED_INSERT_KEY;
			}
			else if (secboot_signed_hdr->tags_number == 2){
				boot_img_type = BOOTIMAGE_SIGNED_NO_INSERT_KEY;
			}
		}
	}

	debugf("set boot_img_type is %d, boot_image_flag:%d\n", boot_img_type, boot_image_flag);

	if(boot_image_flag == TRUE) {
		user_puk_addr = get_user_puk_addr("uboot", header);

		debugf("user_puk_addr:%x\n", user_puk_addr);

		switch(boot_img_type) {
			case BOOTIMAGE_SIGNED_INSERT_KEY:
				//skip check temp
				debugf("call secure_check_bootimage, len:0x%x, len:%d. \n", ((vlr_info_t *) vlr_addr)->length, ((vlr_info_t *) vlr_addr)->length);
				//chk_ret = secure_check_bootimage((uint8_t *) code_addr, ((vlr_info_t *) vlr_addr)->length, (uint8_t *) vlr_addr, (uint8_t *) user_puk_addr);
				chk_ret = secure_check_bootimage((uint8_t *) user_puk_addr, ((vlr_info_t *) vlr_addr)->length, (uint8_t *) vlr_addr, (uint8_t *) user_puk_addr);
				...
				g_verifiedbootstate = v_state_orange;
					...
					lcd_printf("WARNNING: USER KEY VERIFY FAILED!!!\n");
					lcd_printf("WARNNING: SYSTEM WILL SHUTDOWN!!!\n");
					power_down_devices(0);
				}
				break;
			case BOOTIMAGE_UNSIGNED:
			case BOOTIMAGE_SIGNED_NO_INSERT_KEY:
			default:
				chk_ret = secure_check_bootimage((uint8_t *) code_addr, ((vlr_info_t *) vlr_addr)->length, (uint8_t *) vlr_addr, (uint8_t *) puk_addr);
				if(chk_ret == 1) {
					debugf("WARNNING: OEM KEY VERIFY SUCCESS!!!\n");
					//lcd_printf("WARNNING: OEM KEY VERIFY SUCCESS!!!\n");
				g_verifiedbootstate = v_state_green;
				}
				else {
					lcd_printf("WARNNING: OEM KEY VERIFY FAILED!!!\n");
					while(sec_time_count) {
						mdelay(100);
						sec_time_count --;
					}
					lcd_printf("WARNNING: SYSTEM WILL BOOT RIGHT NOW!!!\n");
				g_verifiedbootstate = v_state_red;
				}
		}
	}
	else if(boot_image_flag == FALSE) {
