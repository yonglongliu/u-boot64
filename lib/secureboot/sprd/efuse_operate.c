...
#define NOSEC_ROLLBACK_BLOCK 15
...
int efuse_rollback_version_read(int type)
{
	...
	{
		ret = efuse_read(DDIE_EFUSE, NOSEC_ROLLBACK_BLOCK);
		return ret&NOSEC_ROLL_BIT;
	}
	else
	{
		ret = efuse_read(DDIE_EFUSE, SEC_ROLLBACK_BLOCK);
		return ret&SEC_ROLL_BIT;
	}
...
{	
	...
	if(type)//no_sec_rollback
	{
		ret= efuse_prog(DDIE_EFUSE, NOSEC_ROLLBACK_BLOCK, value&NOSEC_ROLL_BIT);
		printf("efuse_unsec_prog ret =%x,version=%x\n",ret,value);
	}
	else
	{
		ret= efuse_prog(DDIE_EFUSE, SEC_ROLLBACK_BLOCK, value&SEC_ROLL_BIT);
...
int calc_sha256_write_efuse(void){
...
#ifdef CONFIG_SECBOOT
//  -----   generate SHA1 key   -----
  get_spl_sprd_hash((void*)sha256.sha256_char);
...
//  -----   write SHA256 key Efuse   -----
  ...
  ret = efuse_hash_write(sha256_string, 64);
  if(ret < 0){
    debugf("sha256 efuse write Error [%d]\n", ret);
    return SHA1_ERR;
  }
//block2 bit15
