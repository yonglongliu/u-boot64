				...
				disk_partition_t * info)
{
	...
	if (part > le32_to_cpu(gpt_head->num_partition_entries) ||
		...
		debug("%s: *** Invalid partition number %d ***\n",
			__func__, part);
		free(gpt_pte);
		return -1;
	}
				...
				disk_partition_t * info)
{
	...
	for(i=0;i<partition_nums;i++)
	{
		...
		{
			disk_parition[j] = pgpt_pte[i].partition_name[j] & 0xFF;
		}
		if(0 == strcmp(disk_parition, partition_name))
		{
			/* The ulong casting limits the maximum disk size to 2 TB */
			info->start = (ulong) le64_to_cpu((pgpt_pte)[i].starting_lba);
			/* The ending LBA is inclusive, to calculate size, add 1 to it */
			info->size = ((ulong)le64_to_cpu((pgpt_pte)[i].ending_lba) + 1) - info->start;
			info->blksz = dev_desc->blksz;

			sprintf((char *)info->name, "%s", print_efiname(&((pgpt_pte)[i])));
			sprintf((char *)info->type, "U-Boot");
			memcpy(&info->attributes, &pgpt_pte[i].attributes, sizeof(gpt_entry_attributes));
