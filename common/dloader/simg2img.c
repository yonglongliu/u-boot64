...
int write_simg2emmc(char* interface, int  dev, uchar * partname, u8* buf, unsigned long length)
{
	unsigned int i;
	...
	if (0 != get_partition_info_by_name(g_emmc_handle.pdev, partname, &g_emmc_handle.info)) {
	...
	for (i=current_chunks; i<sparse_header.total_chunks; i++) {
		...
		if ((bufferindex + sizeof(chunk_header)) > length) {
			printf("%s:bufferindex(%d) + sizeof(chunk_header)(%d) exceed length\n", __func__, bufferindex, sizeof(chunk_header));
			current_chunks = i;
			break;
		}

		preread_chunk((void*)buf, bufferindex, &chunk_header, sizeof(chunk_header));
		printf("%s:chunk_header.total_sz(%d)\n", __func__, chunk_header.total_sz);
		...
		switch (chunk_header.chunk_type) {
			...
			if ((bufferindex + chunk_header.total_sz) > length) {
				...
				printf("%s:bufferindex(%d)+chunk_header.total_sz(%d) exceed  length(%d) \n", __func__, bufferindex, chunk_header.total_sz, length);
				ret = 1;
			}
			break;
			default:
		...
		switch (chunk_header.chunk_type) {
			...
			printf("%s:write to flash OK,total blk=%d\n", __func__, total_blocks);
			break;
			case CHUNK_TYPE_FILL:
